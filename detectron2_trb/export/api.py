...
from detectron2_trb.config import CfgNode
...
def add_export_config(cfg):
    ...
    Args:
        cfg (CfgNode): a detectron2_trb config
...
class Caffe2Tracer:
    """
    Make a detectron2_trb model traceable with Caffe2 operators.
    This class creates a traceable version of a detectron2_trb model which:
    ...
    def __init__(self, cfg: CfgNode, model: nn.Module, inputs):
        """
        Args:
            cfg (CfgNode): a detectron2_trb config, with extra export-related options
            ...
            model (nn.Module): An original pytorch model. Must be among a few official models
                in detectron2_trb that can be converted to become caffe2-compatible automatically.
...
class Caffe2Model(nn.Module):
    ...
    def __call__(self, inputs):
        """
        An interface that wraps around a Caffe2 model and mimics detectron2_trb's models'
        ...
        on detectron2_trb in order to convert to detectron2_trb's output format.
        """
