...
import detectron2_trb  # noqa F401
from detectron2_trb.structures import Boxes, Instances
from detectron2_trb.utils.env import _import_file
...
def patch_instances(fields):
    """
    A contextmanager, under which the Instances class in detectron2_trb is replaced
    by a statically-typed scriptable class, defined by `fields`.
    See more in `scripting_with_instances`.
    """

    with tempfile.TemporaryDirectory(prefix="detectron2_trb") as dir, tempfile.NamedTemporaryFile(
...
def patch_builtin_len(modules=()):
    """
    Patch the builtin len() function of a few detectron2_trb modules
    to use __len__ instead, because __len__ does not convert values to
    integers and therefore is friendly to tracing.

    Args:
        modules (list[stsr]): names of extra modules to patch len(), in
            addition to those in detectron2_trb.
    """
    ...
    with ExitStack() as stack:
        MODULES = [
            "detectron2_trb.modeling.roi_heads.fast_rcnn",
            "detectron2_trb.modeling.roi_heads.mask_head",
            "detectron2_trb.modeling.roi_heads.keypoint_head",
        ] + list(modules)
...
def patch_nonscriptable_classes():
    """
    Apply patches on a few nonscriptable detectron2_trb classes.
    ...
    from detectron2_trb.modeling.backbone import ResNet, FPN
    ...
    from detectron2_trb.modeling.roi_heads import StandardROIHeads
