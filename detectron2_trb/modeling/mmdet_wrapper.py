...
from detectron2_trb.layers import ShapeSpec
from detectron2_trb.structures import BitMasks, Boxes, ImageList, Instances
...
class MMDetBackbone(Backbone):
    """
    Wrapper of mmdetection backbones to use in detectron2_trb.

    mmdet backbones produce list/tuple of tensors, while detectron2_trb backbones
    produce a dict of tensors. This class wraps the given backbone to produce
    output in detectron2_trb's convention, so it can be used in place of detectron2_trb
    backbones.
    """
...
class MMDetDetector(nn.Module):
    ...
    Input/output formats of this class follow detectron2_trb's convention, so a
    mmdetection model can be trained and evaluated in detectron2_trb.
    """
