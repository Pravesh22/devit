...
from detectron2_trb.export import TracingAdapter
...
class FlopCountAnalysis(fvcore.nn.FlopCountAnalysis):
    """
    Same as :class:`fvcore.nn.FlopCountAnalysis`, but supports detectron2_trb models.
    """
...
def flop_count_operators(model: nn.Module, inputs: list) -> typing.DefaultDict[str, float]:
    ...
    detection models in detectron2_trb.
    ...
    Args:
        model: a detectron2_trb model that takes `list[dict]` as input.
        inputs (list[dict]): inputs to model, in detectron2_trb's standard format.
