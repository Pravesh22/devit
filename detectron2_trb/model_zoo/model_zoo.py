...
from detectron2_trb.config import CfgNode, LazyConfig, get_cfg, instantiate
...
def get_checkpoint_url(config_path):
    ...
    Args:
        config_path (str): config file name relative to detectron2_trb's "configs/"
...
def get_config_file(config_path):
    ...
    cfg_file = pkg_resources.resource_filename(
        "detectron2_trb.model_zoo", os.path.join("configs", config_path)
    )
...
def get(config_path, trained: bool = False, device: Optional[str] = None):
    ...
    Returns:
        nn.Module: a detectron2_trb model. Will be in training mode.

    Example:
    ::
        from detectron2_trb import model_zoo
